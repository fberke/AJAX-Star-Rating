	...
	if (!$static) {
		for ($ncount = 1; $ncount <= $vars ['units']; $ncount++) { // loop from 1 to the number of units
			(!$voted)
				...
				? $rater [] = '<li><a href="?id='.$vars ['id'].'&amp;ip='.$vars ['ip'].'&amp;rating='.$ncount.'&amp;asr=1" title="'.$ncount.' '.$ASR['RATING_LINK_TITLE'].' '.$vars ['units'].'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>'
...
if (isset ($_REQUEST['id']) && isset ($_REQUEST['ip']) && isset ($_REQUEST['rating']) && isset ($_REQUEST['asr'])) {
	//getting the values
	...
	// get votes from DB
	$sql = "SELECT * FROM ".TABLE_PREFIX."mod_asr_ratings WHERE id='$id_sent' ";
	...
	if (!$voted) { //if the user hasn't yet voted, then vote normally...
		if (($vote_sent >= 1 && $vote_sent <= $ratings ['units']) && ($ip == $ip_sent)) { // keep votes within range, make sure IP matches - no monkey business!
...
function ratingBar (
	...
	$sql = "SELECT * FROM ".TABLE_PREFIX."mod_asr_ratings WHERE id='$id' ";
	$db = $database->query ($sql);

	// create DB entry if id doesn't exist yet
	if (!isset ($db) || ($db->numRows() == 0)) {
		$sql = "INSERT INTO ".TABLE_PREFIX."mod_asr_ratings (`id`, `total_votes`, `total_value`, `units`, `unitwidth`) VALUES ('$id', '0', '0', '$units', '$unitwidth')";
		$db = $database->query ($sql);
